		...
		'cloneNode': {
			value: function (deep) {
				...
				switch (this.nodeType) {
					...
					case CONSTANTS.ELEMENT_NODE: clone = new Element(this.tag, this.nodeType); break;
					case CONSTANTS.TEXT_NODE: clone = new Text(this.nodeValue); break;
			...
			'title': {
				get: function () {
					...
					if (!html)
						return undefined;
					for (var i = 0, htmls = html.children, l = htmls.length; i < l; ++i) {
						if (htmls[i].tag === 'title')
							return htmls[i].innerHTML;
					}
					return undefined
				},
				set: function (v) {
					...
					for (var i = 0, htmls = html.children, l = htmls.length; i < l; ++i) {
						if (htmls[i].tag === 'title') {
							html.replaceChild(title, htmls[i]);
							return;
						}
					}
		...
		'drawDocument': {
			value: function (path, format, xmlInfo) {
				if (xmlInfo) {
					...
					data.encoding = (data.encoding || this.charset || 'UTF-8').toUpperCase();
	...
	Object.defineProperties(DOMImplementation.prototype, {
		...
		'createHTMLDocument': {
			value: function (title) {
				var document = this.createDocument(null, 'html', this.createDocumentType('html')),
		...
		'createSVGDocument': {
			value: function () {
				return this.createDocument(null, 'svg', this.createDocumentType('svg'));
			},
			writable: false,
			enumerable: true,
			configurable: false
